...
| `PivotPointLevels` | `PivotPointVariant variant`, `OHLCBar previousPeriodBar` | Levels computed from the previous period's H/L/C. `PivotPointVariant` (`STANDARD`, `CAMARILLA`, `WOODIE`) lives in `commons` as of 0.52.0-alpha (was `org.hatrack.heerwisch.api.spec.PivotPointVariant` through 0.51.0-alpha — breaking FQN move so the driver and `nachtkrapp` share one definition) |
