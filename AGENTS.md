...
- **Exception hierarchy purity**: each module has a single root exception (e.g. `BacktestException`, `DetectionException`, `ChartSpecException`). New error conditions must subclass the module root, carry typed contextual fields, and be documented in the spec.
