...
| Ordering | The driver **normalizes** the feed so the user never suffers for an EODHD-side defect: kept bars are **re-sequenced** into strict ascending time order (EODHD occasionally ships bars out of order, e.g. around DST), and a **duplicated** timestamp is **de-duplicated by keeping the last** bar. Output `time()` values are therefore ascending and unique per the `MarketDataSource` contract (`frau-holle/CLAUDE.md` §2.1). Re-sequencing/de-dup is reported once on the console; it never aborts the fetch |
| Missing required field | a missing/`null` `date` (EOD) or `timestamp` (intraday) is structural and throws `MarketDataSchemaException` naming the absent field. A bar whose `open`/`high`/`low`/`close` is **`null`, blank or a no-data marker** (`"null"`/`"NaN"`/`"NA"`) is a halt / no-trade bar and is **skipped** (not fatal) — real feeds contain these (e.g. ~22 in a 3000-bar 1h BTC window). `volume` is independent: a `null`/absent volume on an otherwise-valid bar maps to `Optional.empty()` and the bar is **kept**. Skips/de-dups are reported once on the console |
