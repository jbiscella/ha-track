...
This is the authoritative specification document for this repo, which hosts three independent Java libraries and two shared kernels, plus reference implementations of pluggable data drivers. All module names are drawn from **Continental Germanic folklore**, deliberately matching the convention of the consumer project (`hütchen` → kobold custodian / mischief-maker from West Germanic tradition). The dualism custodian / trickster is intentional and runs through every name: each library helps the user but can also mislead.
...
| `indicators` | Technical-indicator calculators (SMA, EMA, RSI, MACD, Bollinger, ATR, Stochastic, ADX). Stateless pure functions. Shared kernel for `nachtkrapp`, `heerwisch-jfreechart`, and future consumers. Zero external dependencies (JDK only). | none | `indicators/CLAUDE.md` |
...
| `heerwisch-jfreechart` | Default plotting driver. Consumes `ChartSpec`, produces `ChartImage` (PNG or JPEG). Headless, dual runtime (Lambda-compatible + writable to file by caller). | `heerwisch-api`, `commons`, `indicators`, JFreeChart 1.5.x | `heerwisch-jfreechart/CLAUDE.md` |
...
|---|---|
| Three independent libraries, shared kernels | `heerwisch-*`, `frau-holle`, `nachtkrapp` never import each other; all import `commons`; `heerwisch-jfreechart` and `nachtkrapp` also import the `indicators` calculator kernel |
...
| `indicators` → anything in this repo | no (JDK-only, like `commons`) |
| `heerwisch-api` → `commons` | yes |
| `heerwisch-api` → `heerwisch-jfreechart` | no |
| `heerwisch-api` → `indicators` | no (the API surface holds no calculator) |
| `heerwisch-api` → `frau-holle*` or `nachtkrapp` | no |
| `heerwisch-jfreechart` → `heerwisch-api`, `commons`, `indicators` | yes |
| `heerwisch-jfreechart` → JFreeChart | yes |
| `heerwisch-jfreechart` → `frau-holle*` or `nachtkrapp` | no |
| `frau-holle` → `commons` | yes |
| `frau-holle*` → `indicators` | no (the backtester needs no indicator calculators in v1) |
...
| Indicator calculator code (SMA, EMA, RSI, MACD, …) is needed by both `nachtkrapp` (for primitives like `PriceCrossedAboveMA`) and `heerwisch-jfreechart` (for rendering indicator overlays) | **Resolved in v1.1.** v1 tolerated the duplication; the calculators were extracted into the shared `indicators` module (8th module of the repo) and both `nachtkrapp` and `heerwisch-jfreechart` now consume it. The two pre-extraction calculator sets were verified arithmetically identical, so the move was value-preserving. The indicator reference-value tests now live in `indicators` and cover the calculators directly — they no longer need a cross-module parity test, because there is a single calculator set. See `indicators/CLAUDE.md`. |
| `nachtkrapp` will add chart patterns (head & shoulders, triangles, …) in v2, requiring **swing-pivot detection** (finding local price extrema) | Swing-pivot detection is out of v1 in this repo. When added in v2, it MUST be repaint-safe (lookahead-safety invariant). NB: this is distinct from **pivot-point levels** (support/resistance computed from the prior period's H/L/C), which ARE in scope as of 0.52.0-alpha — see `commons.PivotPoints` and `nachtkrapp`'s `PivotPointRule`. |
