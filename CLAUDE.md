...
| Indicator calculator code (SMA, EMA, RSI, MACD, …) is needed by both `nachtkrapp` (for primitives like `PriceCrossedAboveMA`) and `heerwisch-jfreechart` (for rendering indicator overlays) | v1 tolerates duplication. If drift emerges, extract into a new `indicators` module in v1.1 — naming convention is reserved. **A cross-module test asserting `nachtkrapp` and `heerwisch-jfreechart` compute identical indicator values has no legal home in v1**: no module may depend on both (cross-library dependency edges are forbidden, §2) and both calculator sets live in `internal` packages. Such a parity test is therefore deferred to the v1.1 `indicators` extraction, where the shared module is the natural and only correct host. Until then, each module's calculators are covered by that module's own BDD scenarios. |
