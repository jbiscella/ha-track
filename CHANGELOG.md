...
- **heerwisch-api:** new `LegendEntry(IndicatorPlacement placement, String label, int rgb, Pane pane)` record, and `ChartImage.legend()` returning `List<LegendEntry>` for consumer-side legend rendering. `rgb` is plain 24-bit `0xRRGGBB` (no alpha) — engine-neutral, so `heerwisch-api` carries no `java.awt` coupling. One entry per rendered series, in `ChartSpec.indicators()` insertion order; dual-line indicators (MACD, Stochastic) emit two entries sharing the placement but with distinct label/color (e.g. `MACD(12,26,9)` + `Signal`, `Stoch(14,3,3)` + `%D`). `ChartImage` gains the 5-component canonical form with the 4-arg constructor preserved (empty legend).
