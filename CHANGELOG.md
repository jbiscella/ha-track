...
- **dsl-eval (new module):** stateless DSL condition-evaluation kernel, promoted verbatim from the `wichtelm-app` consumer so multiple consumers can share the exact same evaluation code (an identical condition evaluates identically in each — the parity guarantee is the module's purpose). Root package `org.hatrack.dsl`. Public seam: `ExpressionEvaluator` (recursive-descent arithmetic + English-prose comparators + cross detection) with its injected interfaces `ExpressionEvaluator.Values` and `ExpressionEvaluator.IndicatorSource` and the `Scope` record — this is how each consumer plugs in its own value source. Ships two reference resolvers: `BarIndicatorSource` (resolves `sma`/`ema`/`rsi`/`atr`/`stddev`/`macd_*`/window-aggregate function calls over a `List<OHLCBar>` via the `indicators` module) and `NachtkrappMatchIndex` (maps HA / RSI / MACD / price-vs-MA / pivot boolean primitives to `nachtkrapp` `DetectionRule`s, runs detection once against the closed series, and indexes match instants per call for O(1) per-bar lookup). The prepass takes a wichtelm-agnostic `List<ExpressionRef>` (`(text, timeframeWire)`) instead of a consumer-specific strategy AST. Exceptions `DslEvaluationException` and `IndicatorWarmupException` are ha-track-local (no wichtelm dependency). Deliberately **stateless**: zero references to entry price, position, signal, or backtest orchestration — those stay in the consumer. Depends only on `commons`, `indicators`, `nachtkrapp`, and the JDK. Spec: `dsl-eval/CLAUDE.md`. (The two boolean-primitive parity tweaks landed during review — DECIMAL64 unary negation, DECIMAL64 prepass-literal parsing, and malformed-literal wrapping in `DslEvaluationException` — are part of this first published `dsl-eval`.)

### Compatibility

- Purely additive at the reactor level: a brand-new module (`net.jacopobiscella:dsl-eval`) joins the build; no existing module's public API changed. The repo-wide version moves to `0.56.0-alpha` because `0.55.0-alpha` was already published to Maven Central **without** `dsl-eval`, so the new artifact ships under its own version rather than re-using an already-released coordinate. All reactor modules are re-stamped `0.56.0-alpha` in lockstep, per the shared-version convention. japicmp at 0.x skips the semver gate.
