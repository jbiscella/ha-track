 ...
 *
 * <p>This covers the base indicators {@code sma}, {@code ema}, {@code rsi},
 * {@code atr}, {@code stddev}, the MACD components {@code macd_line},
 * {@code macd_signal}, {@code macd_histogram} and the window aggregates
 * {@code highest_high}, {@code lowest_low}, {@code highest_close},
 * {@code lowest_close} and {@code avg_volume}. {@code stddev} and the window
 * aggregates are computed here because the {@code indicators} module exposes no
 * standalone calculator for them.
 *
 * <p>The boolean pattern primitives ({@code ha_doji}, {@code ha_strong},
 ...
 * {@code macd_zero_cross_up}, {@code macd_zero_cross_down}, the price/MA trend
 * filters and pivots) resolve via the {@link NachtkrappMatchIndex} prepass —
 * each per-bar evaluation is an O(1) membership lookup that returns
 * {@link BigDecimal#ONE} for true, {@link BigDecimal#ZERO} for false, so the
 * boolean primitives compose with the existing numeric-arithmetic / boolean-step
 * pipeline.
 *
 * <p>Stateless: the source holds an immutable bar list plus per-bar diagnostic
 * context and a pre-built match index. It carries no notion of a position,
 * trade, or order; it answers numeric questions about the bars it was given.
 */
public final class BarIndicatorSource implements ExpressionEvaluator.IndicatorSource {
    ...
    public BarIndicatorSource(List<OHLCBar> bars, String source,
                              Instant barTime, long barIndex) {
        this(bars, source, barTime, barIndex, NachtkrappMatchIndex.empty(), "1h");
    }
    ...
    public BigDecimal evaluate(String functionName, List<BigDecimal> arguments) {
        return switch (functionName) {
            ...
            // atr_value is an alias for atr evaluated at this source's current
            // bar. Any "freeze" semantics (e.g. holding ATR constant from a
            // reference bar) are the caller's responsibility, achieved by
            // positioning this source at that bar — the evaluator stays stateless.
        ...
        if (!matchIndex.hasKey(key)) {
            throw fail(name,
                    "pivot primitive '" + name + "(" + level + ")' was not pre-indexed; the "
                            + "expression must declare it statically so the prepass can scan it");
        }
    ...
    private BigDecimal tierB(String name, List<BigDecimal> arguments) {
        ...
        if (!matchIndex.hasKey(key)) {
            // Should not happen — the prepass walks every expression and
            // pre-registers each boolean primitive call. A miss means the call
            // was synthesised at evaluation time (e.g. through a parameter
            // override unseen at setup), which is not supported.
            throw fail(name,
                    "boolean primitive '" + name + "' was not pre-indexed; the expression "
                            + "must declare it statically so the prepass can scan it");
        }
    ...
    private DslEvaluationException fail(String functionName, String message) {
        return new DslEvaluationException(source, 0, barTime, barIndex, functionName, message);
    }
}
