 ...
 *
 * <p>Scope: bare identifiers (resolved through {@link Values}), arithmetic, the
 * English-prose comparison operators, and indicator/primitive function calls
 * resolved through an {@link IndicatorSource}. The evaluator is stateless: it
 * holds only the per-bar diagnostic context ({@code source}, bar time, bar
 * index) and reads every numeric value through the injected seam. It knows
 * nothing about what those identifiers mean to the caller.
 *
 * <p>This class guarantees that an identical expression, fed identical values,
 * evaluates identically across every consumer — that parity is its reason to
 * exist. The arithmetic is {@link BigDecimal} under {@link MathContext#DECIMAL64}
 * throughout; do not substitute {@code double}.
 */
public final class ExpressionEvaluator {
    ...
    public interface IndicatorSource {
        ...
        /**
         * Resolves a pivot primitive on a dedicated boolean-step path that keeps
         * the symbolic level token (R1, S2…) out of the numeric arithmetic
         * pipeline. The default throws: only a source backed by a pre-computed
         * match index (such as {@link BarIndicatorSource}) can resolve a pivot.
         * Pure-arithmetic contexts never reach a pivot step.
         */
    ...
    public ExpressionEvaluator(String source, Instant barTime, long barIndex) {
        this.source = source;
    ...
    public boolean condition(String text, Scope current, Scope previous) {
        ...
        if (operator == null) {
            ...
            // cannot carry, so it is resolved directly against the match index
            ...
            // Bare boolean-valued primitive: "When ha_doji()" or
            // "And rsi_oversold(30)". Evaluate the whole step as an arithmetic
            // expression; the boolean primitives return 1.0 / 0.0 through the
            // IndicatorSource. Any other expression that lacks a comparator
            // yields a numeric value too, which we interpret as truthy iff
            // non-zero — same convention as C / Python.
    ...
    private DslEvaluationException fail(String expression, String message) {
        return new DslEvaluationException(source, 0, barTime, barIndex, expression, message);
    }
