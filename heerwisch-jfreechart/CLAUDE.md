...
### 6.5 Candle display style (`CandleStyle`) — added 0.57.0-alpha

The main pane draws its candles per `ChartSpec.candleStyle()` (`heerwisch-api/CLAUDE.md` §1.11), default `OHLC`:

- **`OHLC`** — the price dataset is built from the supplied series as-is (`OHLCSeries` → raw OHLC candles; `HASeries` → HA candles). Behavior is identical to pre-0.57.
- **`HEIKIN_ASHI`** — when the supplied series is an `OHLCSeries`, the driver derives a Heikin-Ashi series from it (`commons.HeikinAshiCalculator.computeChain`) **for candle drawing only**, via the internal `displaySeries(spec)` helper. When the supplied series is already an `HASeries`, it is drawn unchanged (the style is a no-op).

The display transform is confined to the candle dataset (`buildPriceDataset`) and to the geometry that must align with the drawn candle:

- **Glyph extents** (`computeGlyphExtents`) and the **`EntryExitMarkerAuto` Y anchor** (§8) both read the *displayed* series, so an auto marker sits outside the candle the viewer actually sees — an HA high/low can exceed the raw high/low, which would otherwise tuck the marker inside the body.
- **Overlay indicators** (`addIndicator`) and the **volume pane** (`addVolumeBars`) keep reading `spec.series()` — the real price at its declared `PriceSource` (the source of truth). So an `HEIKIN_ASHI` chart shows standard, real-price RSI / SMA / MACD, and real volume, exactly as on an `OHLC` chart. This is the mainstream-platform convention: Heikin-Ashi is a visualization of price; indicators reflect the true underlying series.

Because `spec.series()` stays an `OHLCSeries`, real-price (`CLOSE`/`OPEN`/`HIGH`/`LOW`) overlays remain valid under `HEIKIN_ASHI` — the api-level V5 price-source check (`heerwisch-api/CLAUDE.md` §6) is not tripped.
...
The driver populates `ChartImage.legend()` with one row **per rendered line**, in `spec.indicators()` insertion order, so consumers can reconstruct the chart's visible series set. `rgb` is the series' rendered color as plain `0xRRGGBB`. A single-line indicator emits one entry labeled by its base (color = the per-placement palette color or the scalar theme color). Multi-line indicators use the `<base>: <role>` colon convention: `MACD` → `<base>: MACD` + `<base>: Signal`; `Stochastic` → `<base>: %K` + `<base>: %D`; `BollingerBands` → `<base>: Upper` + `<base>: Basis` + `<base>: Lower` (all three sharing the placement's palette color — a visually grouped band, but three introspectable lines). `<base>` is the placement's label override or the auto-derived indicator label (§7.1), e.g. `MACD(12,26,9)`, `Stoch(14,3,3)`, `BB(20,2)` — the full parameters stay visible so two same-type placements with different parameters don't collapse to identical labels. `pane()` (always equal to `placement.pane()`) lets consumers group entries into per-pane sections.

Annotation overlays are surfaced separately via `ChartImage.annotationLegend()` (a `List<AnnotationLegendEntry>`, added 0.55.0-alpha) — `legend()` stays indicators-only. The driver emits one `AnnotationLegendEntry(label, rgb)` per **horizontal-line** overlay, in `spec.annotations()` insertion order, with `rgb` the rendered line color as plain `0xRRGGBB`: `PivotPointLevels` → `"Pivot Points (<variant>)"` colored `PIVOT_LEVEL`; `HorizontalLevel` → its own `label` colored by its resolved level color (the semantic `FillColor` mapping, or `HORIZONTAL_LEVEL` when absent); `FibRetracement` → `"Fib Retracement"` colored `FIB_LEVEL`. Glyph (`EntryExitMarker`, `EntryExitMarkerAuto`), text (`BarHighlight`) and band (`TimeRangeHighlight`) annotations are **not** line series and produce no entry.
...
| `PivotPointLevels(variant, previousPeriodBar)` | Levels computed from `previousPeriodBar` via the shared `commons.PivotPoints` calculator (single source of truth, also used by `nachtkrapp`) per the formulas of `variant` (`STANDARD`, `CAMARILLA`, `WOODIE`) — the **complete** canonical set (since 0.52.0-alpha; the pre-0.52 inline calculator omitted STANDARD R3/S3 and WOODIE R2/S2). Rendered as horizontal lines colored `PIVOT_LEVEL`. The main pane's Y auto-range includes every pivot level value (since 0.52.0-alpha, same mechanism as `HorizontalLevel`), so a level outside the price window stays on-chart |
...
| `EntryExitMarkerAuto(time, direction, glyphStyle)` | Same glyph rendering as `EntryExitMarker` (shape from `glyphStyle`, color from `direction`, adaptive half-extents per §8.1) but with the Y position computed by the renderer from the bar at `time`. `LONG_ENTRY` / `SHORT_EXIT` sit at `bar.low − dy · GLYPH_OFFSET_FACTOR_BAR`; `LONG_EXIT` / `SHORT_ENTRY` sit at `bar.high + dy · GLYPH_OFFSET_FACTOR_BAR`. `GLYPH_OFFSET_FACTOR_BAR` defaults to `1.5` (see `ThemeConstants`) — far enough from the bar to read as a separate element, close enough to remain associated. Avoids the visual overlap that occurs when consumers pass `bar.close()` as the explicit price to `EntryExitMarker`. The anchor reads the **displayed** series (§6.5): both `OHLCSeries` (uses `bar.high()` / `bar.low()`) and `HASeries` (uses `bar.haHigh()` / `bar.haLow()`) are supported, and under `CandleStyle.HEIKIN_ASHI` over an `OHLCSeries` the anchor uses the derived HA bar's high/low so the glyph stays outside the drawn HA candle |
| `TimeRangeHighlight(startTime, endTime, fillColor, opacity)` | A shaded background band over `[startTime, endTime]` drawn via JFreeChart's `IntervalMarker` on the domain axis, placed at `Layer.BACKGROUND` so it sits behind candles and indicator lines. Base color from `fillColor` — direction-oriented: `LONG_POSITION` → `TIME_RANGE_LONG`, `SHORT_POSITION` → `TIME_RANGE_SHORT`, `NEUTRAL` → `TIME_RANGE_NEUTRAL`, `CAUTION` → `TIME_RANGE_CAUTION`; outcome-oriented: `WIN` → `TIME_RANGE_WIN`, `LOSS` → `TIME_RANGE_LOSS`, `OPEN` → `TIME_RANGE_OPEN`. The outcome-oriented constants currently share RGBs with their direction-oriented counterparts (`WIN` ≡ `TIME_RANGE_LONG` green, `LOSS` ≡ `TIME_RANGE_SHORT` red, `OPEN` ≡ `TIME_RANGE_NEUTRAL` blue/grey) but the renderer may differentiate later without API change. Per-instance `opacity` (`[0, 1]`) is applied as the fill alpha. No outline is drawn |
